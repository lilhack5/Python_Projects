...
def addToList(self):
    ...
    var_lname = var_lname.strip() # This will ensure that the first character each word is capitalized
    var_fname = var_fname.title()
    var_lname = var_lname.title()
    var_fullname = ("{} {}".format(var_fname,var_lname)) # combine our normialized names into a fullname
    print("var_fullname: {}".format(var_fullname))# silent just for development
...
def onDelete(self):
    var_select = self.lstList1.get(self.lstList1.curselection()) # Listbox'x selected value
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
        ...
        count = cur.fetchone()[0]
        if count > 1:
            confirm = messagebox.askokcancel("Delete Confirmaiton", "All information associated with, ({}) \nwill be permenantly deleted from the database. \n\nProceed with the deletion request?".format(var_select()))
            if confirm:
                conn = sqlite3.connect('phonebook.db')
                with conn:
                    cursor = conn.cursor()
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            # the database...meaning, there area no changes to update.
